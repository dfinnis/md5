...
static uint64_t	*pad(char *input, size_t *msg_len)
{
	...
	padded = NULL;
	(*msg_len) = (((g_byte_len + 16) * 8) / 1024) + 1;
	if ((padded = ft_calloc((*msg_len) * 16, 64)) == NULL)
		error_exit("memory allocation failure");
	((uint8_t*)padded)[g_byte_len] = 0x80;
	ft_memcpy(padded, input, g_byte_len);
	i = -1;
	while (++i < g_byte_len + 1)
		padded[i] = swap_endian64(padded[i]);
	padded[((((*msg_len) * 1024) - 128) / 64) + 1] = g_byte_len * 8;
	return (padded);
}
